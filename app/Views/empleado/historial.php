...
<div id="contenedor-historial">
    <?php if (empty($pedidos)): ?>
        <div class="text-center py-5"
            style="background: rgba(0,0,0,.1); border: 1px dashed var(--borde); border-radius: 16px;">
            <i class="bi bi-clock-history" style="font-size: 40px; color: var(--texto-3); opacity: 0.3;"></i>
            <p class="mt-3" style="font-size: 11px; color: var(--texto-3); text-transform: uppercase; letter-spacing: 2px;">
                Sin registros históricos</p>
        </div>
    <?php else: ?>
        <div class="row">
                    ...
                    <div class="emp-task-card task-done">
                        ...
                        <div class="task-meta" style="margin-bottom: 10px; padding-bottom: 10px;">
                            <div class="task-meta-item">
                                ...
                                <?= isset($pedido['fechacompletado']) ? date('d M Y H:i', strtotime($pedido['fechacompletado'])) : '---' ?>
                            </div>
                        </div>

                        <div class="task-actions">
                            <span
                                style="font-size: 10px; color: var(--texto-3); font-weight: 700; letter-spacing: 0.5px;">ALMACENADO</span>
                            <button class="task-primary-btn btn-view" style="padding: 6px 15px; font-size: 12px;"
                                onclick="verDetalleSolicitud(<?= $pedido['id'] ?>)">
