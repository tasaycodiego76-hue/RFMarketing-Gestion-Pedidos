...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const INFO_TIPOS = {
    adaptacion: {
      ...
      desc: "Ajustes menores o cambios de formato a un diseño existente.",
      dias: "2",
      equipo: "Diseñador Gráfico",
    },
    creacion: {
      titulo: "Creación de Arte",
      desc: "Diseño original desde cero para piezas visuales publicitarias.",
      dias: "4",
      equipo: "Diseñador + Director de Arte",
    },
    creacion_editorial: {
      titulo: "Creación de Editorial",
      desc: "Maquetación de revistas, boletines o guías de múltiples páginas.",
      dias: "7",
      equipo: "Equipo Editorial",
    },
    adaptacion_editorial: {
      titulo: "Adaptación de Editorial",
      desc: "Ajustes a publicaciones editoriales ya existentes.",
      dias: "7",
      equipo: "Equipo Editorial",
    },
    creacion_video: {
      titulo: "Creación de Video",
      desc: "Producción audiovisual completa (guion, edición, post-producción).",
      dias: "7",
  ...
  async function cargarServicios() {
    ...
    listaServicios.innerHTML =
      '<div class="text-center p-4"><span class="spinner-border text-warning"></span></div>';
    try {
  ...
  async function obtenerPedidos() {
    ...
    try {
      ...
      tablaPedidosBody.innerHTML = datos
        .map((pedido, index) => {
  ...
  function irAlPaso(paso) {
    ...
    document.getElementById("form-titulo-servicio").textContent =
      `Paso ${paso}: ${titulos[paso - 1]}`;
    ...
    document
      ...
      .classList.toggle("d-none", paso === 3);
    document
  ...
  function validarYPasar() {
    ...
    if (pasoActual === 1) {
      ...
      if (idServ === "0" && !getIdVal("titulo_personalizado"))
      ...
      if (!modoConsultivo && !getVal('[name="objetivo"]'))
        errores.push("El objetivo de comunicación es obligatorio.");
      if (!modoConsultivo && !tipo)
        errores.push("El tipo de requerimiento es obligatorio.");
      ...
      if (fechaEnt) {
        ...
        if (fechaIng < fechaMin)
          errores.push(
            `La fecha mínima para este servicio es de ${diasMin} días hábiles.`,
          );
        // Si eligió Sábado o Domingo.
        if (fechaIng.getDay() === 0 || fechaIng.getDay() === 6)
          errores.push("No se permiten entregas en fines de semana.");
      } else {
    ...
    if (pasoActual === 2) {
      ...
      if (!esServicioConsultivo(nombreServicioSeleccionado)) {
        ...
        if (checkedVals("canales[]").length === 0)
          errores.push("Selecciona al menos un canal de difusión.");
        if (checkedVals("formatos[]").length === 0)
          errores.push("Selecciona al menos un formato.");
      }

      // Validación de materiales de referencia.
      if (!selectMateriales.value) {
        errores.push(
          "Debe seleccionar si cuenta con materiales de referencia.",
        );
      } else if (selectMateriales.value === "1") {
        // Si dijo que sí tiene, debe haber subido un archivo O puesto un link de Drive/Wetransfer.
        const tieneArchivos =
          archivosSeleccionados.length > 0 || inputArchivos?.files?.length > 0;
        const tieneUrl = getVal('[name="url_referencia"]') !== "";

        if (!tieneArchivos && !tieneUrl) {
          errores.push(
            "Debe subir un archivo o proporcionar una URL de referencia.",
          );
        }
      }
      ...
      if (urlVal) {
        ...
        if (!urlRegex.test(urlVal)) {
          errores.push(
            "Debe ingresar una URL válida para la referencia (ej: https://drive.google.com/...)",
          );
  ...
  function generarResumen() {
    ...
    document.getElementById("res-materiales").innerHTML = tieneMat
      ? '<span class="text-success">✓ Sí, incluye materiales</span>'
      : '<span class="text-muted">No incluye materiales</span>';
    // Mostramos el link solo si el usuario escribió uno.
    document
      .getElementById("res-link-wrap")
      .classList.toggle("d-none", !getVal('[name="url_referencia"]'));
  ...
  formNuevoPedido?.addEventListener("submit", (e) => {
    ...
    if (!progressLine) {
      ...
      progressLine.style.cssText =
        "position:absolute; bottom:0; left:0; height:3px; background:rgba(0,0,0,0.3); width:0%; transition:width 0.2s ease;";
    ...
    if (getVal('[name="formato_otros"]'))
      fd.append("formato_otros", getVal('[name="formato_otros"]'));
    if (getVal('[name="url_referencia"]'))
      fd.append("url_subida", getVal('[name="url_referencia"]'));
  ...
  modalFormularioDetalleEl?.addEventListener("hidden.bs.modal", () => {
    formNuevoPedido.reset();
    pasoActual = 1;
    archivosSeleccionados = [];
    listaArchivos.innerHTML = "";
